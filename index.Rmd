...
Similarly, the `visNetwork` package requires the nodes to be a data frame. The original nodes data frame in the `visNetwork` object originally contained two columns, an "id" column and a "label" column that contained the state names for the nodes. However, we also added a new column for the color of each node, where color corresponded to the proportion of annual COVID cases per state population. The `eigScalePal` function creates a gradient of five colors from the `colorRampPalette` function, and the `mutate` command breaks the COVID cases proportions column values into five sections and assigns each value to one out of the five colors.
...
In order to create an interactive network in a Shiny app, the function, `visNetwork`, can be used within the `renderVisNetwork` function. The `visNetwork` function needs two arguments, the nodes and the edges, and using the pipe function, there are many other functions that can be used to further customize the network, such as being able to select a node and have its connecting edges highlighted using the `visOptions` function or being able to specify the color of the edges and the different color they should change to when selected using the `visEdges` function. 
