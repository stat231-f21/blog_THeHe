...
cut(annual_cases$covid_prop, breaks = 10)
...
restaurants_nodes <- restaurants_visNetwork$nodes %>%
  ...
  mutate(color = eigScalePal(5)[cut(covid_prop, breaks = 10)]) %>%
...
states <- c("AK", "CT", "SD", "AR", "LA")
# 
stay_at_home_orders_final <- stay_at_home_orders_final %>%
  filter(state_one %in% states,
         state_two %in% states) 
...
```{r}
 visNetwork(stay_at_home_orders_nodes, stay_at_home_orders_edges, height = "700px", width = "100%", 
...
active_nodes <- reactive({
  switch(input$restriction,
         "rest" = restaurants_nodes,
         "bar" = bars_nodes,
         "mask" = mask_mandates_nodes,
         "gatban" = gathering_ban_nodes,
         "stayhome" = stay_at_home_orders_nodes)
})
  
active_edges <- reactive({
  switch(input$restriction,
         "rest" = restaurants_edges,
         "bar" = bars_edges,
         "mask" = mask_mandates_edges,
         "gatban" = gathering_ban_edges,
         "stayhome" = stay_at_home_orders_edges
         )
})
